			...
			var band = false;
				...
				if(l > 0){
					if(data >= 0 && data <= 10){
						...
						$('#'+name).parent("div").addClass('success');
						band = true;
						
						var pos = data.indexOf('.');
						if((pos != -1) && (l-pos)>3){
							document.getElementById(name).value = data.substr(0,pos+3);	
						}
								
					}else{
						$('#'+name).parent("div").removeClass('success');	
		...
		//FUNCION NUEVA
		function opcionlista(name){
			var indice = document.getElementById(name).selectedIndex;
			if( indice == null || indice == 0 ) {
				document.getElementById(name+"t").innerHTML = "Debe seleccionar una opcion";
				$('#'+name+'1').removeClass('success');	
				$('#'+name+'1').addClass('error');
			  return false;
			}else{
		...
		function IMAGEN(name){
			var data = document.getElementsByName(name).item(0).value;
			var tama = ((document.getElementById(name).files[0].size)/1024);
			var l = data.length;
			var band = false;
			
			if((data.substr((l-4),l) == ".jpg" || data.substr((l-4),l) == ".png" || data.substr((l-5),l) == ".jpeg" || data.substr((l-4),l) == ".gif")){
				if(tama > 2048){
					document.getElementById(name+"t").innerHTML = "No debe exceder el tamaño de 2 MB";
					$('#'+name+'1').removeClass('success');	
					$('#'+name+'1').addClass('error');
				}else{
		...
		var toutput =  (day<10 ? '0' : '') + day + '/' + (month<10 ? '0' : '') + month + '/' +d.getFullYear();
		...
		function FECHA(name,mini,maxi){
			...
			var tmaxf =  (day<10 ? '0' : '') + day + '/' + (month<10 ? '0' : '') + month + '/' +(d.getFullYear()+maxi);
			var tminf =  (day<10 ? '0' : '') + day + '/' + (month<10 ? '0' : '') + month + '/' +(d.getFullYear()-mini);
			
			if(!/Invalid|NaN/.test(new Date(data)) && l == 10){
				if(data > maxf){
					document.getElementById(name+"t").innerHTML = "Fecha máxima: "+tmaxf;
					$('#'+name+'1').removeClass('success');	
					$('#'+name+'1').addClass('error');
				}else if(data < minf){
					document.getElementById(name+"t").innerHTML = "Fecha mínima: "+tminf;
					$('#'+name+'1').removeClass('success');	
					$('#'+name+'1').addClass('error');
				}else{
					document.getElementById(name+"t").innerHTML = "ÉXITO";
				...
				document.getElementById(name+"t").innerHTML = "Fecha no válida";
				$('#'+name+'1').removeClass('success');	
				$('#'+name+'1').addClass('error');
			}
			return band;
		...
		function VERIFICAR(name,mini,maxi,name2){
			...
			if(l < mini || (/(['"])/.test(data))){
				...
				if(!(/([0-9])/.test(data)) || !(/([A-Z])/.test(data)) || !(/([a-z])/.test(data))){
					if(!(/([0-9])/.test(data))){
						document.getElementById(name+"t").innerHTML = "Introduzca números";
					}else if(!(/([A-Z])/.test(data))){
						document.getElementById(name+"t").innerHTML = "Introduzca letras mayúsculas";
					}else{
						document.getElementById(name+"t").innerHTML = "Introduzca letras minúsculas";
					}
					$('#'+name+'1').removeClass('success');	
					$('#'+name+'1').addClass('error');
					$('#'+name2+'1').removeClass('success');	
